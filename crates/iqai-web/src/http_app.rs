//! HTTP uygulaması: Axum router, sayfalar ve API — `build_router` entegrasyon testleri için.
...
use crate::{
    ai,
    api_json::{
        api_error_with_extras_and_request_id, api_error_with_request_id,
    },
    api_spec,
...
/// TFAI-Q05: JSON hata gövdesinde `request_id` — HTTP `X-Request-Id` ile aynı (`tower-http`).
fn x_request_id_from_extensions(req: &Request) -> Option<&str> {
    req.extensions()
        .get::<RequestId>()
...
/// G-03: CORS — `config.web.cors_allow_origins` doluysa liste; yoksa permissive.
fn cors_layer() -> CorsLayer {
    let origins = iqai_core::AppConfig::load()
        .and_then(|c| c.web.as_ref().and_then(|w| w.cors_allow_origins.clone()));
    match origins {
        Some(list) if !list.is_empty() => {
            let headers: Vec<http::HeaderValue> =
...
/// Tüm HTTP rotaları + CORS — `tests/http_smoke` ve dış entegrasyon için.
pub fn build_router() -> Router {
    Router::new()
        .merge(api_spec::router())
        .route("/", get(index))
        ...
        .route("/api/ai-explanations", get(api_ai_explanations))
        .route(
            "/api/config",
            get(api_get_config).post(api_update_config),
        )
        // TFAI §3.3 / Q05: istek korelasyonu — `X-Request-Id` (yoksa UUID); yanıtta aynı başlık.
        .layer(PropagateRequestIdLayer::x_request_id())
        .layer(SetRequestIdLayer::x_request_id(MakeRequestUuid::default()))
        .layer(cors_layer())
}

/// Binary giriş noktası — log init + `8080` dinleme.
pub async fn run_server() -> Result<()> {
    ...
    println!("API şeması: http://localhost:8080/api/docs  (OpenAPI: /api/openapi.yaml)");
...
/// TFAI-O06: Prometheus exposition (HTML `/metrics` ile çakışmaz).
fn resolve_trade_db_path_for_metrics() -> String {
    std::env::var("IQAI_TRADING_DB").ok().or_else(|| {
        iqai_core::AppConfig::load().and_then(|c| c.trading.and_then(|t| t.db_path))
    }).unwrap_or_else(|| "data/trades.db".to_string())
}

async fn metrics_prometheus() -> impl IntoResponse {
    let path = resolve_trade_db_path_for_metrics();
    match TradeDb::open(Some(path.as_str())) {
        Ok(db) => match iqai_core::render_prometheus_sli(&db) {
            ...
            Err(e) => (
                ...
                format!("prometheus render error: {e}\n"),
            )
                .into_response(),
        },
        Err(_) => {
            let body = iqai_core::render_prometheus_sli_minimal(false);
            Response::builder()
                .status(StatusCode::OK)
                .header(
                    header::CONTENT_TYPE,
...
async fn api_pnl_symbols(Query(params): Query<PnlQuery>, req: Request) -> impl IntoResponse {
    let rid = x_request_id_from_extensions(&req);
    let mode = params
        .mode
        .as_deref()
        ...
        .and_then(|c| c.trading.and_then(|t| t.db_path))
    {
        Some(p) => p,
        None => {
            return axum::Json(api_error_with_extras_and_request_id(
                "trading.db_path not set",
                json!({ "symbols": [] }),
                rid,
            ));
        }
    ...
    match db.get_symbol_pnl_stats(mode) {
        ...
        Err(e) => axum::Json(api_error_with_extras_and_request_id(
            e.to_string(),
            json!({ "symbols": [] }),
            rid,
        )),
...
/// `symbols: []` iken varsayılan ETHUSDT/BTCUSDT kullanılır — ayrıntı: `docs/API_Q_ANALYSIS.md`.
/// Piyasa: Binance Futures. GET /api/q-analysis
async fn api_q_analysis_all(req: Request) -> impl IntoResponse {
    let rid = x_request_id_from_extensions(&req);
...
async fn api_q_analiz_detections(Query(params): Query<DetectionsQuery>, req: Request) -> impl IntoResponse {
    let rid = x_request_id_from_extensions(&req);
...
async fn api_q_analiz_snapshot(Query(params): Query<SnapshotQuery>, req: Request) -> impl IntoResponse {
    let rid = x_request_id_from_extensions(&req);
    let app_cfg = iqai_core::AppConfig::load().unwrap_or_default();
    let symbol = params
        .symbol
    ...
    let tf = match Timeframe::from_str(&tf_str) {
        Some(t) => t,
        None => {
            return axum::Json(api_error_with_request_id(
                format!("Geçersiz timeframe: {}", tf_str),
                rid,
            ));
        }
    };
...
#[derive(Debug, Deserialize)]
struct AiExplanationsQuery {
    symbol: Option<String>,
    limit: Option<u32>,
}

/// TFAI-O08: Kayıtlı AI açıklamaları (denetim / geri izleme).
async fn api_ai_explanations(Query(params): Query<AiExplanationsQuery>, req: Request) -> impl IntoResponse {
    let rid = x_request_id_from_extensions(&req);
    let path = match iqai_core::AppConfig::load().and_then(|c| c.trading.and_then(|t| t.db_path.clone())) {
        Some(p) => p,
        None => {
    ...
    let lim = params.limit.unwrap_or(30) as usize;
    match db.list_ai_explanations(params.symbol.as_deref(), lim) {
        Ok(rows) => axum::Json(serde_json::json!({ "explanations": rows })),
        Err(e) => axum::Json(api_error_with_extras_and_request_id(
            e.to_string(),
            json!({ "explanations": [] }),
            rid,
...
async fn api_analysis_snapshots(Query(params): Query<AnalysisSnapshotsQuery>, req: Request) -> impl IntoResponse {
    let rid = x_request_id_from_extensions(&req);
    let path = match iqai_core::AppConfig::load().and_then(|c| c.trading.and_then(|t| t.db_path.clone())) {
        Some(p) => p,
        None => {
...
async fn api_analysis_snapshots_report(
    Query(params): Query<AnalysisSnapshotsQuery>,
    req: Request,
) -> impl IntoResponse {
    let rid = x_request_id_from_extensions(&req);
    let path = match iqai_core::AppConfig::load().and_then(|c| c.trading.and_then(|t| t.db_path.clone())) {
    ...
    let ai_text = if let Some(ref ai) = app_cfg.ai {
        if ai.enabled == Some(true) {
            ...
            ai::interpret_big_picture(base, model, symbol, &context).await
        } else {
            None
        }
    } else {
    ...
    if app_cfg.ai_persist_explanations() {
        if let Some(ref text) = ai_text {
            let model = app_cfg
                .ai
                .as_ref()
                .and_then(|a| a.model.as_deref())
                .unwrap_or("llama2");
            let prompt = ai::build_big_picture_prompt(symbol, &context);
            let prompt_hash = iqai_core::sha256_hex(prompt.as_bytes());
            let context_hash = iqai_core::sha256_hex(context.as_bytes());
            let fp = iqai_core::fingerprint_analysis_snapshots_for_audit(&snapshots);
            let refs = serde_json::json!(snapshots
                .iter()
            ...
            if let Err(e) = db.insert_ai_explanation(
                "big_picture",
                model,
                ai::PROMPT_TEMPLATE_VERSION_BIG_PICTURE,
                &prompt_hash,
                &context_hash,
                Some(&fp),
                Some(symbol),
                ...
                log::warn!("ai_explanations kayıt hatası: {}", e);
            }
        }
    }
...
fn trend_arrow_label(t: Option<i32>) -> &'static str {
    match t {
        Some(-1) => "↓",
        Some(1) => "↑",
        _ => "—",
...
fn format_tmr_triple(t: i32, m: i32, r: i32) -> String {
    if t == 0 && m == 0 && r == 0 {
        "—/—/—".to_string()
    } else {
        format!("{}/{}/{}", t, m, r)
...
fn format_snapshots_for_ai(symbol: &str, snapshots: &[AnalysisSnapshotRecord]) -> String {
    ...
    for s in snapshots {
        ...
        let lt = trend_arrow_label(s.local_trend);
        let gt = trend_arrow_label(s.global_trend);
        let tmr_str = format_tmr_triple(trend_pts, mom_pts, rr_pts);
...
async fn api_update_config(
    ...
    // `Extension` (FromRequestParts) `Json` (body) öncesi olmalı — aksi halde `Handler` türemez.
    let rid = request_id.header_value().to_str().ok();
        ...
        match serde_json::to_string_pretty(&new_cfg) {
            Ok(contents) => {
                if let Err(e) = fs::write(&path, contents) {
                    ...
                    return axum::Json(api_error_with_request_id(e.to_string(), rid));
                }
                axum::Json(serde_json::json!({ "ok": true }))
            }
            Err(e) => {
                eprintln!("Config serialize error: {}", e);
                axum::Json(api_error_with_request_id(e.to_string(), rid))
            }
        }
    } else {
        axum::Json(api_error_with_request_id("Config path not found", rid))
    }
}
...
async fn api_chart(Query(params): Query<ChartParams>) -> impl IntoResponse {
    ...
    let app_cfg_chart = iqai_core::AppConfig::load();
    let verbose_chart_poll = app_cfg_chart
        .as_ref()
        .map(|c| c.logging_verbose_chart_poll())
        .unwrap_or(false);
    let chart_log = move |msg: String| {
        if verbose_chart_poll {
            log::info!(target: "iqai_chart", "{}", msg);
        } else {
            log::debug!(target: "iqai_chart", "{}", msg);
        }
    };

    let max_bars = app_cfg_chart
        .as_ref()
        .and_then(|c| c.data.as_ref().and_then(|d| d.max_bars))
        .unwrap_or(10_000);
    let mut buffer = CandleBuffer::new();
    ...
    if use_tv {
        let client = if let Some(script) = tv_script {
            chart_log(format!("[TV] Backend: Subprocess (Python) → {}", script));
            ...
            chart_log(format!("[TV] Backend: HTTP → {}", base));
            TvConnectorClient::with_exchange(base, tv_exchange)
        } else {
            chart_log("[TV] Backend: Auto (tradingview-rs varsa onu, yoksa Native Rust WebSocket)".to_string());
        ...
        for tf in timeframes {
            match client.fetch_klines(symbol, tf, max_bars).await {
                ...
                Ok(_) => chart_log(format!("[TV] {} {:?}: 0 bar (boş)", symbol, tf)),
                Err(e) => chart_log(format!("[TV] {} {:?} hatası: {}", symbol, tf, e)),
            }
        }
    } else {
        ...
        for tf in timeframes {
            match client.fetch_klines(symbol, tf, max_bars).await {
                Ok(candles) if !candles.is_empty() => buffer.update(tf, candles),
                Ok(_) => chart_log(format!("[Binance] {} {:?}: 0 bar (boş)", symbol, tf)),
                Err(e) => chart_log(format!("[Binance] {} {:?} hatası: {}", symbol, tf, e)),
            }
        }
    }
