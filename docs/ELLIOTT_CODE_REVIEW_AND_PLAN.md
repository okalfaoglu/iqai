...
| A4 | **Üçgen W2 yasağı** | Spesifikasyon: üçgen W2 olamaz. **Kısmen:** `triangle_wave2_context_blocked` + geçersiz sayım işareti; tam bağlam (W4 vs B, zigzag–üçgen önceliği) için audit sürüyor. |
...
- [x] Üçgen: **W2 yasağı** — `elliott::triangle_wave2_context_blocked` (8 pivot heuristik) + `try_triangle` içinde `validation_ok` / `elliott_invalidate_hint` + birim testler (`triangle_wave2_context_tests`, `try_triangle_w2_tests`). **Kalan:** zigzag önceliği yüzünden üçgenin hiç seçilmediği senaryolar + tam W4/B pozisyon ayrımı.
...
- [x] XM / TradingView tarzı PDF × kod karşılaştırması (TODO/DONE matrisi): [`XMST_TRADINGVIEW_EW_PDF_VS_KOD.md`](./XMST_TRADINGVIEW_EW_PDF_VS_KOD.md) — PDF sayfa eşlemesi §4 şablonu elle doldurulacak.
